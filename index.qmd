...
extract_metadata <- function(filename) {
    matches <- regexec("([A-Z_a-z0-9]+)_([MF])_([A-Z_a-z]+)_D(\\d+)(?:_([A-Z_a-z]+))?.txt", filename)
    ...
    processing <- ifelse(match_details[[1]][6] == "", "IP", match_details[[1]][6])

    return(list(mouseline = mouseline, sex = sex, condition = condition, processing = processing, day = day))
}
```
...
meta <- do.call(rbind, lapply(basename(files.list), function(f) {
...
ggplot(mds.df, aes(x = MDS1, y = MDS2, color = mouseline, shape = sex, size = processing)) +
...
for (ml in unique(se$mouseline)) {
    ...
    design <- model.matrix(~ sex + cond_day + processing, data = colData(se.sub))
    keep <- filterByExpr(dge, design)
    dge <- dge[keep, , keep.lib.sizes = FALSE]
...
ggplot(mds.df, aes(x = MDS1, y = MDS2, color = cond_day, shape = sex, size=processing)) +
    geom_point(alpha = 0.8) +
    scale_size_discrete(range = c(3, 5)) +
